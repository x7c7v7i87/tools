...
func GetNiceName() string {
	rand.Seed(time.Now().UnixNano())     //设置随机数种子
	var first string                     //名
	for i := 0; i <= rand.Intn(3); i++ { //随机产生2位或者3位的名
		first = fmt.Sprint(firstName[rand.Intn(firstNameLen-1)])
	}
	mapsc := chinese.RelatedWords(first)
	mapscLen := len(mapsc[first])
	var lenRRR int
	var nicename string
	if mapscLen == 0 {
		nicename = randomdata.LastName() //+ randomdata.FirstName(randomdata.Male)
	} else {
		lenRRR = rand.Intn(mapscLen)
		nicename = mapsc[first][lenRRR]
	}
	return nicename
}
